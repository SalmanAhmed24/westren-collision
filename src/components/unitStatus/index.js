...
function UnitStatus({ title }) {
  ...
  const fetchData = async () => {
    setLoadingFlag(true);
    const res = await axios.get(`${apiRouth.prodPath}/api/unitStatus`);
    if (res.data && res.data.error == false) {
      setData(res.data.unitStatus);
  ...
  const handleAddUnit = async (itemObj) => {
    try {
      const res = await axios.post(
        `${apiRouth.prodPath}/api/unitStatus/addunitStatus`,
        itemObj
      );
      console.log(res);
      handleClose();
    ...
    <main className="unitMake-wrap">
      ...
      <section className="tabular-data">
        {loadingFlag ? (
          ...
          <p>No Unit Status value found please Add values</p>
        ) : (
          <UnitTable
            title={title}
