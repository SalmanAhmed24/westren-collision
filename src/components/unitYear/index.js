...
function UnitYear({ title }) {
  ...
  const fetchData = async () => {
    setLoadingFlag(true);
    const res = await axios.get(`${apiRouth.prodPath}/api/unitYear`);
    if (res.data && res.data.error == false) {
      setData(res.data.unitYear);
  ...
  const handleAddUnit = async (itemObj) => {
    try {
      const res = await axios.post(
        `${apiRouth.prodPath}/api/unitYear/addunitYear`,
        itemObj
      );
      console.log(res);
      handleClose();
